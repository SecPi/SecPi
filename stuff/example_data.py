...
from tools.db.objects import Worker, Param, Action, Notifier, Sensor, Zone, Setup, LogEntry
...
cam_params = [
	...
	Param(key='data_path', value='/var/tmp/secpi/worker_data', description='Path to store the images.', object_type="action"),
]
...
email_params = [
	...
	Param(key='text', value='Your SecPi raised an alarm. Please check the attached files.', description='Text for the mail.', object_type="notifier"),
	Param(key='data_dir', value='/var/tmp/secpi/alarms', description='Directory to fetch the files from.', object_type="notifier"),
	Param(key='smtp_address', value='mail.secpi.local', description='SMTP Server to send mails from.', object_type="notifier"),
	...
	Param(key='smtp_security', value='STARTTLS', description='Security setting for SMTP Server (can be STARTTLS, SSL, NOSSL, NOAUTH_NOSSL, NOAUTH_STARTTLS, NOAUTH_SSL).', object_type="notifier")
]
...
sensor_gpio = Sensor(name="GPIO Sensor", description="This is a simple sensor which reads a GPIO Port", module="gpio_sensor", cl="GPIOSensor", worker=p_worker, params=sensor1_params)
sensor_tcp = Sensor(name="TCP Sensor", description="This is a sensor which listens on a TCP port.", module="tcpportlistener", cl="TCPPortListener", worker=p_worker, params=sensor2_params)
sensor_temp = Sensor(name="Temperature Sensor", description="A sensor that checks if a temperature is in a defined range.", module="temperature_sensor", cl="TemperatureSensor", worker=p_worker, params=sensor3_params)
